...
class MazeEnvironment3D(BaseEnvironment):
    """Implements the environment for an RLGlue environment
    ...
    def __init__(self):
        ...
        self.maze_dim = [6, 6, 6]
        self.obstacles = [[0, 2, 0], [1, 2, 0], [2, 2, 0], [3, 2, 0], [4, 2, 0], [5, 2, 0],
        ...
        self.start_state = [2, 0, 0]
        self.end_state = [0, 5, 0]
    ...
    # check if current state is within the gridworld and return bool
    def out_of_bounds(self, row, col, z):
        if row < 0 or row > self.maze_dim[0]-1 or col < 0 or col > self.maze_dim[1]-1 or z < 0 or z > self.maze_dim[2]-1:
            return True
        else:
            return False

    # check if there is an obstacle at (row, col)
    def is_obstacle(self, row, col, z):
        if [row, col, z] in self.obstacles:
            return True
        else:
            return False

    def get_observation(self, state):
        #return state[0]*self.maze_dim[1]*self.maze_dim[2] + state[1]*self.maze_dim[2] + state[2]
        return state[0] + state[1]*self.maze_dim[0] + state[2]*self.maze_dim[0]*self.maze_dim[1]
    ...
    def env_step(self, action):
        ...
        # update current_state with the action (also check validity of action)
        if action == 0: # up
            if not (self.out_of_bounds(row-1, col, z) or self.is_obstacle(row-1, col, z)):
                self.current_state = [row-1, col, z]

        elif action == 1: # right
            if not (self.out_of_bounds(row, col+1, z) or self.is_obstacle(row, col+1, z)):
                self.current_state = [row, col+1, z]
            ...
            if not (self.out_of_bounds(row, col, z+1) or self.is_obstacle(row, col, z+1)):
                self.current_state = [row, col, z+1]                
                
        elif action == 5: # bottom
            if not (self.out_of_bounds(row, col, z-1) or self.is_obstacle(row, col, z-1)):
                self.current_state = [row, col, z-1]                
...
class MazeEnvironment3Dshortcut(BaseEnvironment):
    """Implements the environment for an RLGlue environment
    ...
    def env_step(self, action):
        ...
        self.timesteps += 1
        if self.timesteps == self.change_at_n:
            self.obstacles = [[0, 2, 0], [1, 2, 0], [2, 2, 0],            [4, 2, 0], [5, 2, 0],
                              [0, 2, 1], [1, 2, 1], [2, 2, 1], [3, 2, 1], [4, 2, 1], [5, 2, 1],
                              [0, 2, 2], [1, 2, 2], [2, 2, 2], [3, 2, 2], [4, 2, 2], [5, 2, 2],
                              [0, 2, 3], [1, 2, 3], [2, 2, 3], [3, 2, 3], [4, 2, 3], [5, 2, 3],
                              [0, 2, 4], [1, 2, 4], [2, 2, 4]           , [4, 2, 4], [5, 2, 4],
